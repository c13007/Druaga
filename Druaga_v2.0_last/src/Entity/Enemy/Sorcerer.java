...
public class Sorcerer extends Entity {
    ...
    public Sorcerer(EntityManager em, int x, int y) {
        super(em, 17, x, y);
    }
    ...
    public void init() {
        try {
            ImageIcon icon = new ImageIcon(getClass().getResource("/images/entity/sorcerer.png"));
            image = icon.getImage();
    ...
    protected void attack() {
        ...
        if (!em.getMap().isHit(x, y, this.dir)) {
            em.addEntity(new RedMagic(em, getX(), getY(), getDir()));
