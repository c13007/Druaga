...
public class WizardGhost extends Entity{
    
    Random rand;
    private LOS los;
    private Position positionOld;
    
    public WizardGhost(EntityManager em, int x, int y){
        super(em,19,  x, y);
        los = new LOS(em,x,y);
    ...
    public void init() {
        try{
            ImageIcon icon = new ImageIcon(getClass().getResource("/images/entity/wizardghost.png"));
            image = icon.getImage();
    	...
    	if(!freezing){
            ...
            if(r < 6){
            	...
            	if(!moving){
	            	...
	            	if(dix == 0 && diy == -1) this.dir = Entity.UP;
	            	else if(dix == 0 && diy == 1) this.dir = Entity.DOWN;
	            	else if(dix == -1 && diy == 0) this.dir = Entity.LEFT;
	            	else if(dix == 1 && diy == 0) this.dir = Entity.RIGHT;
